...
fn clone_experiment() -> Vec<i32> {
    ...
    thread::spawn( move || {
        for i in 1..6 {
            tx.send(i).expect("There was an issue sending your tranmission from within the new thread.");
            thread::sleep(Duration::from_secs(1));
        }
    });

    for i in 1..11 {
        tx2.send(i).expect("There was an issue sending your tranmission from within the main thread.");
        thread::sleep(Duration::from_secs(1));
    }
